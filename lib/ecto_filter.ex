defmodule EctoFilter do
  ...
  @type search_time ::
          %{
            startTime: NaiveDateTime.t(),
            endTime: NaiveDateTime.t(),
            now: NaiveDateTime.t(),
            tz: integer
          }
          | %{
  ...
  def format_search_time(search) do
    ...
    cond do
      ...
      search["interval"] ->
        interval =
          if is_binary(search["interval"]),
            do: String.to_integer(search["interval"]),
            else: search["interval"]
      ...
      search["startTime"] ->
        startTime =
          NaiveDateTime.from_iso8601!(search["startTime"] <> " 00:00:00")
          |> Timex.beginning_of_day()

        endTime =
          NaiveDateTime.from_iso8601!(search["endTime"] <> " 00:00:00")
          |> Timex.end_of_day()
  ...
  def gen_dynamic(dy, :and, field, :eq, search_term),
    do: dynamic([t, c], field(c, ^field) == ^search_term and ^dy)

  def gen_dynamic(dy, :and, field, :is_null, true),
    do: dynamic([t, c], is_nil(field(c, ^field)) and ^dy)

  def gen_dynamic(dy, :and, field, :is_null, false),
    do: dynamic([t, c], not is_nil(field(c, ^field)) and ^dy)

  def gen_dynamic(dy, :and, _, :all, _), do: dy

  def gen_dynamic(dy, :or, field, :eq, search_term),
    do: dynamic([t, c], field(c, ^field) == ^search_term or ^dy)
  ...
  def format_search_field(search_field) when is_binary(search_field),
    do: String.to_atom(search_field)
  ...
  def patch_by_hours([rec | reset], tz, acc) do
    hour =
      Timex.shift(rec.inserted_at, seconds: tz)
      |> Map.get(:hour)
